...
def hbl_login():
    ...
    headers = {
        ...
        'Origin': 'https://ib1.himalayanbank.com/',
        'Referer': 'https://ib1.himalayanbank.com/',
    ...
    response = requests.post(
        'https://ib1.himalayanbank.com/expressBanking-client-web/api/Customer/auth',
        cookies=cookies,
        headers=headers,
        json=json_data,
    )
...
def loan_status(headers):
    response = requests.get('https://ib1.himalayanbank.com/expressBanking-client-web/api/Account/loans', headers=headers)
    data = json.loads(response.text)
    if data:
        st.write(data[0]['loanType'], " - ", data[0]['reference']," - Rs." ,data[0]['amount'])
    else:
        st.write("Loan amount not found")
...
def view_statement(bank_name, to_date, from_date, headers):
    ...
    response = requests.post(
        'https://ib1.himalayanbank.com/expressBanking-client-web/api/Account/AcStatement',
...
def set_extract(bank_name, to_date, from_date):
    ...
    response = requests.post(
        'https://ib1.himalayanbank.com/expressBanking-client-web/api/Account/downloadAcStatement/EXCEL',
        cookies=st.session_state.cookies,
...
def combine(to_date, from_date):
    ...
    format_statement('od', to_date, from_date).to_excel(writer, sheet_name='J9')
    format_statement('call', to_date, from_date).to_excel(writer, sheet_name='TG')
    format_statement('usd48', to_date, from_date).to_excel(writer, sheet_name='TG USD')
    format_statement('usd72', to_date, from_date).to_excel(writer, sheet_name='J9 USD')
...
# Main function to render the HBL page
def HBL_page():
    st.title("HBL Bank Statement Manager")
    ...
    if st.session_state.logged_in:
        ...
        st.write("## View Loan Outstanding")
        if st.button("Loan Balance"):
            loan_status(headers)
